...
\author{Mohamed Hozien \and \textit{Partner Name}}
\date{May 2026}
...
\begin{center}
  ...
  \texttt{historical CSV} $\rightarrow$ \texttt{ALS Trainer} $\rightarrow$
  \texttt{als\_model/} $\rightarrow$ \texttt{Streaming app}\\
  \texttt{Producer} $\rightarrow$ \texttt{Kafka(interactions, 3p)} $\rightarrow$
  \texttt{Spark Structured Streaming} $\rightarrow$
  \{\texttt{recs}, \texttt{alerts}\} topics + parquet sinks\\
  \texttt{Streamlit dashboard} polls parquet sinks every 2\,s.}}
\end{center}
% Replace the box above with figures/architecture.png once the diagram is drawn.
...
keep the best configuration. \textit{Insert final RMSE table here after running
\texttt{train\_als.py}.}

\section{Streaming Component}
\subsection{Kafka Setup}
Kafka 2.12-3.7.0 runs in KRaft single-node mode from a portable folder
(\texttt{\$KAFKA\_DIR/bin/kafka-server-start.sh}). The
...
\texttt{maxOffsetsPerTrigger=2000}, Kryo serializer,
\texttt{spark.sql.shuffle.partitions=8}, factor matrices kept in driver memory
(no per-batch I/O). \textit{Insert measured p50/p95 histogram here after the
5-minute run.}

\section{Results}
\textit{Replace placeholders with the values produced by your run:}
\begin{itemize}[noitemsep]
  \item ALS RMSE (final): \texttt{<value>}
  \item Average events/sec sustained: \texttt{<value>}
  \item Latency p50 / p95: \texttt{<ms>} / \texttt{<ms>}
  \item Total alerts triggered in a 5-minute demo: \texttt{<value>}
\end{itemize}
...
See \texttt{README.md}. Short version:
...
python src/kafka_producer.py --mode synthetic --rate 80 &
spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.13:4.1.1 \
             --master "local[3]" src/streaming_app.py &
